...
#import "/components/index.typ": (
  classnames, colors, docs-category, docs-section, fonts, icon, paged-heading-offset, prose-styling, search-box,
  ty-pill, use-icon,
)
...
#let symbol-list-entries(mod, prefix) = {
  ...
  for (name, s) in dictionary(mod) {
    ...
    for (variant, value, deprecation) in info.variants {
      ...
      let alternates = info.variants.map(((variant, ..)) => complete(variant)).filter(v => v != full-name)
