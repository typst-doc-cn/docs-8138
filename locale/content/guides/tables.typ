...
If you want to look up a detail of how tables work, you should also @table[check out their reference page]. And if you are looking for a table of contents rather than a normal table, the reference page of the @outline[`outline` function] is the right place to learn more.

= #short-or-long[Basic Tables][How to create a basic table?] <basic-tables>
In order to create a table in Typst, use the @table[`table` function]. For a basic table, you need to tell the table function two things:
...
This example shows how to call, configure, and populate a table. Both the column count and cell contents are passed to the table as arguments. The @function[argument list] is surrounded by round parentheses. In it, we first pass the column count as a named argument. Then, we pass multiple @content[content blocks] as positional arguments. Each content block contains the contents for a single cell.
...
It is best to wrap the header row of your table in the @table.header[`table.header` function]. This clarifies your intent and will also allow Typst to make the output more @guides:accessibility[accessible] to users with a screen reader:
...
You could also write a show rule that automatically @strong[strongly emphasizes] the contents of the first cells for all tables. This quickly becomes useful if your document contains multiple tables!
...
Congratulations, you have created your first table! Now you can proceed to @guides:tables:column-sizes[change column sizes], @guides:tables:strokes[adjust the strokes], @guides:tables:fills[add striped rows], and more!

= #short-or-long[Column Sizes][How to change the column sizes?] <column-sizes>
...
Here, we have passed our list of column lengths as an @array[array], enclosed in round parentheses, with its elements separated by commas. The first two columns are automatically sized, so that they take on the size of their content and the third column is sized as `{1fr}` so that it fills up the remainder of the space on the page. If you wanted to instead change the second column to be a bit more spacious, you could replace its entry in the `columns` array with a value like `{6em}`.

= #short-or-long[Captions And References][How to caption and reference my table?] <captions-and-references>
A table is just as valuable as the information your readers draw from it. You can enhance the effectiveness of both your prose and your table by making a clear connection between the two with a cross-reference. Typst can help you with automatic @ref[references] and the @figure[`figure` function].
...
= #short-or-long[Fills][How to get a striped table?] <fills>
...
- A function that takes the horizontal coordinate `x` and the vertical coordinate `y` of a cell and returns its fill. We can use this to create horizontal stripes or @grid.cell[checkerboard patterns].
...
This example shows a book club reading list. The line `{fill: (rgb("EAF2F5"),  none)}` in `table`'s set rule is all that is needed to add striped columns. It tells Typst to alternate between coloring columns with a light blue (in the @color.rgb[`rgb`] function call) and nothing (`{none}`). Note that we extracted all of our styling from the `table` function call itself into set and show rules, so that we can automatically reuse it for multiple tables.

Because setting the stripes itself is easy we also added some other styles to make it look nice. The other code in the example provides a dark blue @guides:tables:stroke-functions[stroke] around the table and below the first line and emboldens the first row and the column with the book title. See the @guides:tables:strokes[strokes] section for details on how we achieved this stroke configuration.
...
== #short-or-long[Fill Override][Manually overriding a cell's fill color] <fill-override>
Sometimes, the fill of a cell needs not to vary based on its position in the table, but rather based on its contents. We can use the @table.cell[`table.cell` element] in the `table`'s parameter list to wrap a cell's content and override its fill.
...
= #short-or-long[Strokes][How to adjust the lines in a table?] <strokes>
...
- Do you want to style all tables in your document, irrespective of their size and content? Use the `table` function's @table.stroke[stroke] argument in a set rule.
- Do you want to customize all lines in a single table? Use the `table` function's @table.stroke[stroke] argument when calling the table function.
...
We will go over all of these options with examples next! First, we will tackle the `table` function's @table.stroke[stroke] argument. Here, you can adjust both how the table's lines get drawn and configure which lines are drawn at all.
...
This makes the table lines a bit less wide and uses a bluish gray. You can see that we added a width in point to a color to achieve our customized stroke. This addition yields a value of the @stroke[stroke type]. Alternatively, you can use the dictionary representation for strokes which allows you to access advanced features such as dashed lines.
...
This turns off all vertical strokes and leaves the horizontal strokes in place. To achieve the reverse effect (only vertical strokes), set the stroke argument to `{(y: none)}` instead.

@guides:tables:stroke-functions[Further down in the guide], we cover how to use a function in the stroke argument to customize all strokes individually. This is how you achieve more complex stroking patterns.

== #short-or-long[Individual Lines][Adding individual lines in the table] <individual-lines>
If you want to add a single horizontal or vertical line in your table, for example to separate a group of rows, you can use the @table.hline and @table.vline elements for horizontal and vertical lines, respectively. Add them to the argument list of the `table` function just like you would add individual cells and a header.
...
== #short-or-long[Stroke Functions][Complex document-wide stroke customization] <stroke-functions>
...
== #short-or-long[Double Stroke][How to achieve a double line?] <double-stroke>
Typst does not yet have a native way to draw double strokes, but there are multiple ways to emulate them, for example with @tiling[tilings]. We will show a different workaround in this section: Table gutters.
...
= #short-or-long[Alignment][How to align the contents of the cells in my table?] <alignment>
...
When using the `table` function's align argument, you can choose between three methods to specify an @alignment[alignment]:
...
You can find an example of using `table.cell` to change a single cell's alignment on @table.cell[its reference page].

= #short-or-long[Merge Cells][How to merge cells?] <merge-cells>
...
The example also contains a black header (created with `table`'s `fill` argument) with white strokes (`table`'s `stroke` argument) and white text (set by the `table.cell` set rule). Finally, we align all the content of all table cells in the body in the center. If you want to know more about the functions passed to `align`, `stroke`, and `fill`, you can check out the sections on @alignment[alignment], @guides:tables:stroke-functions[strokes], and @guides:tables:fills[striped tables].

This table would be a great candidate for fully automated generation from an external data source! Check out the @guides:tables:importing-data[section about importing data] to learn more about that.

= #short-or-long[Rotate Table][How to rotate a table?] <rotate-table>
...
- If you want to rotate only the table but not the other content of the page and the page itself, use the @rotate[`rotate` function] with the `reflow` argument set to `{true}`.
- If you want to rotate the whole page the table is on, you can use the @page[`page` function] with its `flipped` argument set to `{true}`. The header, footer, and page number will now also appear on the long edge of the page. This has the advantage that the table will appear right side up when read on a computer, but it also means that a page in your document has different dimensions than all the others, which can be jarring to your readers.
...
What we have here is a two-column document on ISO A5 paper with page numbers on the bottom. The table has six columns and contains a few customizations to @guides:tables:strokes[stroke], alignment and spacing. But the most important part is that the table is wrapped in a call to the `rotate` function with the `reflow` argument being `{true}`. This will make the table rotate 90 degrees counterclockwise. The reflow argument is needed so that the table's rotation affects the layout. If it was omitted, Typst would lay out the page as if the table was not rotated (`{true}` might become the default in the future).
...
If you have placed your table inside of a @figure[figure], it becomes unable to break across pages by default. However, you can change this behavior. Let's take a look:
...
A figure automatically produces a @block[block] which cannot break by default. However, we can reconfigure the block of the figure using a show rule to make it `breakable`. Now, the figure spans multiple pages with the headers and footers repeating.

= #short-or-long[Importing Data][How to import data into a table?] <importing-data>
Often, you need to put data that you obtained elsewhere into a table. Sometimes, this is from Microsoft Excel or Google Sheets, sometimes it is from a dataset on the web or from your experiment. Fortunately, Typst can load many @reference:data-loading[common file formats], so you can use scripting to include their data in a table.
...
We have loaded our file (assuming we named it `moore.csv`) and @reference:scripting:bindings[bound it] to the new variable `moore`. This will not produce any output, so there's nothing to see yet. If we want to examine what Typst loaded, we can either hover the name of the variable in the web app or print some items from the array:
...
With the arguments `{(0, 3)}`, the @array.slice[`slice`] method returns the first three items in the array (with the indices 0, 1, and 2). We can see that each row is its own array with one item per cell.
...
The example above uses a for loop that iterates over the rows in our CSV file and returns an array for each iteration. We use the for loop's @reference:scripting:bindings[destructuring] capability to discard all but the last two items of each row. We then create a new array with just these two. Because Typst will concatenate the array results of all the loop iterations, we get a one-dimensional array in which the year column and the number of transistors alternate. We can then insert the array as cells. For this we use the @arguments:spreading[spread operator] (`..`). By prefixing an array, or, in our case an expression that yields an array, with two dots, we tell Typst that the array's items should be used as positional arguments.

Alternatively, we can also use the @array.map[`map`], @array.slice[`slice`], and @array.flatten[`flatten`] array methods to write this in a more functional style:
...
In this example, we first drop the header row from the data since we are adding our own. Then, we discard all but the last two columns as above. We do this by @reference:scripting:bindings[destructuring] the array `m`, discarding all but the two last items. We then convert the string in `count` to a floating point number, calculate its logarithm and store it in the variable `log`. Finally, we round it to two digits, convert it to a string, and store it in the variable `rounded`. Then, we return an array with `year` and `rounded` that replaces the original row. In our table, we have added our custom header that tells the reader that we've applied a logarithm to the values. Then, we spread the flattened data as above.

We also styled the table with @guides:tables:fills[stripes], a @guides:tables:individual-lines[horizontal line] below the first row, @guides:tables:alignment[aligned] everything to the right, and emboldened the first column. Click on the links to go to the relevant guide sections and see how it's done!

= #short-or-long[Table And Grid][What if I need the table function for something that isn't a table?] <table-and-grid>
